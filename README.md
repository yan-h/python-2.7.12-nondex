...
This version of Python can be run in three nondeterminism modes:
ONE - Shuffling subroutines always start with the same seed. Argument: -no
FULL - Shuffling subroutines start with random seeds. Argument: -nf
NONE - Regular, unmodified Python. Argument: -nx

Objects/dictobject.c is modified to randomize the order of dictionary iteration.
Modules/posixmodule.c was modified to shuffle the return value of os.listdir().
Python/nondeterminism.c contains common subroutines for implementing nondeterminism.
libtest/ contains a hacky script to test git libraries in bulk.

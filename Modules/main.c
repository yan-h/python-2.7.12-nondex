...
#define BASE_OPTS "3bBc:dEhiJm:n:OQ:RsStuUvVW:xX?"
...
-n ms  : set nondeterminism. m is f (FULL), o (ONE), or x (OFF).\n\ 
         s is an optional seed. e.g. nf123\n\
...
Py_Main(int argc, char **argv)
{
    ...
    char nd_mode = nd_mode_env ? nd_mode_env[0] : 'x';
    ...
    while ((c = _PyOS_GetOpt(argc, argv, PROGRAM_OPTS)) != EOF) {
        ...
        /* Read modes for nondeterminism */
        case 'n':
            if (_PyOS_optarg[0] == '\0') {
                fprintf(stderr,
                    "-n option should be `-nf<seed>` or `-no<seed>` "
                    " where <seed> is an optional integer");
                return usage(2, argv[0]);
            }
